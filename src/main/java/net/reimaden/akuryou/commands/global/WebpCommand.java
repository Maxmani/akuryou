...
public class WebpCommand {

    private static final String name = "webp";
    ...
    public static void register(List<CommandData> commandData) {
        OptionData url = new OptionData(OptionType.STRING, option1, "The URL of the WebP file you want to convert.");
        OptionData file = new OptionData(OptionType.ATTACHMENT, option2, "The WebP file you want to convert.");
        OptionData format = new OptionData(OptionType.STRING, option3, "The file format you want. Ignore for PNG.")
                .addChoice("PNG", "png")
                .addChoice("JPG", "jpg")
                .addChoice("GIF", "gif");
        commandData.add(Commands.slash(name, "Convert WebP files to other formats.")
        ...
        if (event.getName().equals(name)) {
            ...
            if (!fileUrl.isEmpty()) {
                if (fileUrl.matches("^(https?)://[-a-zA-Z0-9+&@#/%?=~_|!:,.;]*[-a-zA-Z0-9+&@#/%=~_|]")) {
                    if (fileUrl.endsWith(".webp")) {
                        ImageConverter.convertUrl(event, fileUrl, outputFormat);
                    } else {
                        event.reply("No WebP image found!").setEphemeral(true).queue();
                    }
                } else {
            ...
            if (attachment != null) {
                if (attachment.isImage()) {
                    ...
                    if (fileExtension != null && fileExtension.equals("webp")) {
